...
const literalFromText = (text: string): Literal => {
  ...
  if (trimmed === "null") return { kind: "null", text: trimmed };
  if (trimmed === "undefined") return { kind: "undefined", text: trimmed };
  if (trimmed.startsWith("{")) return { kind: "object", text: trimmed };
  if (trimmed.startsWith("[")) return { kind: "array", text: trimmed };
 ...
 */
const typeStringFrom = (
  typeNode: ts.TypeNode | undefined,
  ctx: WalkContext,
): TypeString => {
  if (!typeNode) return { text: "", refs: [] };
...
const extractFunction = (
  ...
  return {
    ...
    typeRefs: [],
    doc,
    tags: Object.keys(doc.customTags).map((t) => t.replace(/^@/, "")),
...
const extractInterfaceMembers = (
  node: ts.InterfaceDeclaration,
  ctx: WalkContext,
): Member[] =>
  node.members.map((m): Member => {
    ...
    if (ts.isMethodSignature(m)) {
      return {
        name,
        kind: "method",
        signature: m.getText(ctx.sourceFile).replace(/;$/, ""),
...
const extractEnum = (node: ts.EnumDeclaration, ctx: WalkContext): VSymbol => {
  const name = node.name.text;
  const doc = docOrEmpty(node, ctx.sourceFile);
...
export const collectNames = (
  ...
  for (const stmt of sourceFile.statements) {
    if (ts.isInterfaceDeclaration(stmt) || ts.isTypeAliasDeclaration(stmt) || ts.isClassDeclaration(stmt) || ts.isEnumDeclaration(stmt)) {
      if (stmt.name) names.set(stmt.name.text, makeId(modulePath, stmt.name.text));
